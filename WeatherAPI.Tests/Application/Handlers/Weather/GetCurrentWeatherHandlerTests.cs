...
public class GetCurrentWeatherHandlerTests
{
    ...
    private readonly Mock<ILogger<GetCurrentWeatherHandler>> _loggerMock = new(); 
    [Fact]
    public async Task Handle_ValidLocation_ReturnSuccessWithDto()
    {
        ...
        var handler = new GetCurrentWeatherHandler(weatherServiceMock.Object, _loggerMock.Object);
        //Act
    ...
    {
        weatherServiceMock.Setup(s => s.GetCurrentWeatherAsync(It.IsAny<string>())).ThrowsAsync(new Exception());
        var query = new GetCurrentWeatherQuery("Ha Noi");
        var handler = new GetCurrentWeatherHandler(weatherServiceMock.Object, _loggerMock.Object);
        //Act
        var result = await handler.Handle(query, CancellationToken.None);
