...
namespace WeatherAPI.Tests.Application.Handlers.Weather;

public class GetForecastWeatherHandlerTests
{
    private readonly Mock<IWeatherService> weatherServiceMock = new();
    private readonly Mock<ILogger<GetForecastWeatherHandler>> _loggerMock = new(); 
    [Fact]
    public async Task Handle_ValidLocation_ReturnSuccessWithDto()
    {
        ...
        var handler = new GetForecastWeatherHandler(weatherServiceMock.Object, _loggerMock.Object);
    ...
    {
        weatherServiceMock.Setup(s => s.GetForecastWeatherAsync(It.IsAny<string>())).ThrowsAsync(new Exception());
        var query = new GetForecastWeatherQuery("Ha Noi");
        var handler = new GetForecastWeatherHandler(weatherServiceMock.Object, _loggerMock.Object);
        //Act
        var result = await handler.Handle(query, CancellationToken.None);
